  ...
  def has_ancestry options = {}
    ...
    scope :ordered_by_ancestry, lambda {
      if %w(mysql mysql2 sqlite postgresql).include?(connection.adapter_name.downcase) && ActiveRecord::VERSION::MAJOR >= 5
        reorder("coalesce(Arel.sql(#{connection.quote_table_name(table_name)}.#{connection.quote_column_name(ancestry_column)}, ''))")
      else
    ...
    scope :ordered_by_ancestry_and, lambda { |order|
      if %w(mysql mysql2 sqlite postgresql).include?(connection.adapter_name.downcase) && ActiveRecord::VERSION::MAJOR >= 5
        reorder("coalesce(Arel.sql(#{connection.quote_table_name(table_name)}.#{connection.quote_column_name(ancestry_column)}, ''), #{order})")
      else
